...
function SiteFooter({ className = '' }) {
  return (
    <footer
      className={`border-t border-[#e2e4e9] bg-[#f5f6f8] py-6 dark:border-[#1e2029] dark:bg-[#0a0b0e] ${className}`}
    >
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-center gap-4 px-4 sm:flex-row sm:gap-8 sm:px-6 lg:px-8">
        <nav className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-slate-600 dark:text-slate-400">
          <Link to="/privacy" className="font-medium hover:text-slate-900 dark:hover:text-slate-200">
            Privacy
          </Link>
          <a
            href={GITHUB_URL}
            ...
            className="inline-flex items-center gap-1.5 font-medium hover:text-slate-900 dark:hover:text-slate-200"
          >
            <GitHubIcon />
            GitHub
          </a>
